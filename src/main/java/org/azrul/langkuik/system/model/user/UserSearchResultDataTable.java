 ...
 * @author azrulm8
 */
public class UserSearchResultDataTable<C,W> extends DataTable<C,W> {

    public UserSearchResultDataTable(FindAnyEntityQuery<C,W> parameter, Class<C> classOfBean, DataAccessObject<C,W> dao, int noBeansPerPage,/*final Set<String> currentUserRoles,*/ final EntityRight entityRight, final PageParameter pageParameter) {
    ...
    protected void createSearchPanel(final Class<C> classOfBean, /*final Set<String> currentUserRoles,*/ final EntityRight entityRight, final PageParameter<W> pageParameter) throws UnsupportedOperationException, SecurityException, FieldGroup.BindException {
        ...
        Map<Integer, DataElementContainer> elementContainers = beanUtils.getOrderedFieldsByRank(classOfBean);
        ...
        Button searchBtn = new Button(pageParameter.getLocalisedText("form.general.button.search"), new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent event) {
                ...
                //do query
                Collection<C> allData = dao.runQuery(daoQuery, orderBy, asc, currentTableIndex, itemCountPerPage, UserSecurityUtils.getCurrentTenant(), pageParameter.getWorklist());
                if (allData.isEmpty()) {
    ...
    public void deleteEntities(Collection<C> currentBeans, W worklist) throws EntityIsUsedException {

        dao.delete(currentBeans);
        Collection<C> data = dao.runQuery(daoQuery, orderBy, asc, currentTableIndex, itemCountPerPage, UserSecurityUtils.getCurrentTenant(), worklist);
        if (data.isEmpty()) {
            data = new ArrayList<>();
            try {
        ...
        itemContainer.refreshItems();
        bigTotal = dao.countQueryResult(daoQuery, UserSecurityUtils.getCurrentTenant(), worklist);
